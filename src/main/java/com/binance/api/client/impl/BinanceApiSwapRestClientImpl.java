...
public class BinanceApiSwapRestClientImpl implements BinanceApiSwapRestClient
{
	...
	public Liquidity getPoolLiquidityInfo(String poolId)
	{
		...
		List<Liquidity> liquidities = executeSync(
				binanceApiService.getPoolLiquidityInfo( poolId, BinanceApiConstants.DEFAULT_RECEIVING_WINDOW, timestamp ) );
	...
	{
		long timestamp = System.currentTimeMillis();
		return executeSync( binanceApiService.addLiquidity( poolId, asset, quantity, BinanceApiConstants.DEFAULT_RECEIVING_WINDOW,
				timestamp ) );
	}
	...
	{
		long timestamp = System.currentTimeMillis();
		return executeSync( binanceApiService.removeLiquidity( poolId, type, asset, shareAmount,
	...
	{
		long timestamp = System.currentTimeMillis();
		return executeSync( binanceApiService.getPoolLiquidityOperationRecords( poolId, limit,
				BinanceApiConstants.DEFAULT_RECEIVING_WINDOW, timestamp ) );
	...
	{
		long timestamp = System.currentTimeMillis();
		List<LiquidityOperationRecord> records = executeSync( binanceApiService.getLiquidityOperationRecord( operationId,
				BinanceApiConstants.DEFAULT_RECEIVING_WINDOW, timestamp ) );
	...
	{
		long timestamp = System.currentTimeMillis();
		return executeSync( binanceApiService.swap( quoteAsset, baseAsset, quoteQty, BinanceApiConstants.DEFAULT_RECEIVING_WINDOW,
				timestamp ) );
	}
	...
	{
		long timestamp = System.currentTimeMillis();
		List<SwapHistory> history = executeSync(
