...
public class DepthCacheExample
{
	...
	private void initializeDepthCache()
	{
		...
		{
			asks.put( new BigDecimal( ask.getPrice() ), new BigDecimal( ask.getVolume() ) );
		}
		...
		{
			bids.put( new BigDecimal( bid.getPrice() ), new BigDecimal( bid.getVolume() ) );
		}
	...
	private void applyPendingDeltas(final List<DepthEvent> pendingDeltas)
	{
		...
		final Consumer<DepthEvent> drainPending = newEvent -> {
			...
			// after the snapshot.
			pendingDeltas.stream().filter( e -> e.getFinalUpdateId() > lastUpdateId ) // Ignore any updates
																						// before the snapshot
	...
	private void updateOrderBook(NavigableMap<BigDecimal, BigDecimal> lastOrderBookEntries, List<OrderBookEntry> orderBookDeltas)
	{
		for ( OrderBookEntry orderBookDelta : orderBookDeltas )
		{
			...
			BigDecimal qty = new BigDecimal( orderBookDelta.getVolume() );
	...
	{
		new DepthCacheExample( "BTCUSDT,LTCUSDT" );
	}
